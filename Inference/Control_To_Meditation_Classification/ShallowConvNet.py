...
from Inference.Control_To_Meditation_Classification.DataStructureParser import dataStructureParser
...
def test_network(num_test_iterations = 1000):
...
def inference(input_sample):
    # y_out = session.run(output, feed_dict={x: input_sample})
    # return y_out
    with tf.gfile.GFile("C:/Users/JimBobKingJambo/PycharmProjects/PinnacleProject/Inference/Control_To_Meditation_Classification/frozen_shallow_convnet_meditation.bytes", "rb") as f:
    ...
    with tf.Session(graph = graph) as sess:
        y_out = sess.run(y, feed_dict={x : input_sample})
        return y_out
